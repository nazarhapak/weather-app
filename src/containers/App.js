...
class App extends Component {
  constructor () {
    ...
    this.state = {
      ...
      temp_max: "",
      temp_min: "",
      wind: {speed: 3, deg: 349, gust: 1},
      feels_like: 0,
      visibility: 0,
      humidity: 0,
      hourly_forecast: [{hour: 0, weather: ["Clear", "clear"], temperature: 0}],
      daily_forecast: [
        {weather: Array(2), temp_max: 15, temp_min: 2, rain: 0, weekday: "sunday"},
        {weather: Array(2), temp_max: 9, temp_min: 7, rain: '39%', weekday: "monday"},
        {weather: Array(2), temp_max: 7, temp_min: 4, rain: '21%', weekday: "tuesday"},
        {weather: Array(2), temp_max: 10, temp_min: 2, rain: 0, weekday: "wednesday"}
      ],
      sunrise: "",
      sunset: "",
      map_url: "",
  ...
  onKeyDown = (event) => {
    this.removeAutoplaceOverflow()
    if (event.key === 'Enter') {
      this.getLocationCoordinates(event.target.value);
    }
  }

  getLocationCoordinates(location) {
    fetch("https://weather-application-ww10.onrender.com/location-coordinates", {
      method: 'post', 
  ...
  getCurrentWeatherData (latitude, longitude) {
    ...
    .then(data => {
      ...
      if (sunrise) { sunrise = convert_unix(sunrise, timezone).slice(0, 5); }
      if (sunset) { sunset = convert_unix(sunset, timezone).slice(0, 5); }

      const time = convert_unix(data.dt, timezone).slice(0, 5);
      const hour = time.split(":")[0];

      this.setState({weather, temp, sunrise, sunset, wind, hour, feels_like, visibility, humidity, time})
    })
  }
  ...
  getHourlyForecast (latitude, longitude) {
    ...
    .then(data => {
      ...
      const hourly_forecast = data.list.slice(0, 10).map(prediction => {
        ...
        let rain = 0;
        if (prediction.rain) {rain = Math.round(prediction.pop * 100) + "%"}
      ...
      const hourly_forecast = () => {
        data.list.forEach ( prediction => {
          ...
          weekdays.forEach( weekday => {
            ...
            let rain = 0;
            if (prediction.rain) {rain = prediction.pop * 100 + "%"}

            if (day.toLowerCase() === weekday) {
              week_forecast[weekday].push({weather, temp_min, temp_max, rain})
            }
          })
        })
      }
        ...
        weekdays.forEach(weekday => {
          if (week_forecast[weekday].length === 8) {
            ...
            week_forecast[weekday].forEach(prediction => {
              ...
              if (prediction.rain) {rain_percentage = prediction.rain}
            ...
            const weather = [highest_occurance(main), description[main.indexOf(highest_occurance(main))]]
            ...
            daily_forecast.push({weather, temp_max, temp_min, rain, weekday})
          }
        })
      }
  ...
  getPrecipitationMap (latitude, longitude) {
    function tiles(latitude, longitude, zoom) {
      ...
      let y = Math.round((1 - Math.asinh(Math.tan(latitude_radians)) / Math.PI) / 2.0 * num);
  ...
  render () {
    ...
    return (
      <div className='application'>
        ...
        <CurrentWeather name={name} weather={weather} temp={temp} temp_max={temp_max} temp_min={temp_min}/>
        <div className='widgets'>
          <HourlyForecast forecast={hourly_forecast} sunrise={sunrise} sunset={sunset} weather={weather} temp={temp} hour={hour}/>
          <Map precipitation_url={precipitation_url} map_url={map_url}/>
          <DailyForecast forecast={daily_forecast} weather={weather} temp_max={temp_max} temp_min={temp_min}/>
          <Wind wind={wind}/>
